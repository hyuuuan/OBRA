...
| **X14** | **The layer was set and the mask never was.** X3 put the tread on its own collision layer so the player could not stand on it, and left `collision_mask` at the default 1 — which is the layer the player is on. Godot pairs two bodies when EITHER side matches, so the pair existed anyway: the apo passed through the plank exactly as intended and shoved it the length of the paddy on the way. It read as buoyancy. | An exception against the player specifically — the mask cannot drop layer 1, because the terrain, the placed drawings and the player all share it, and a plank that ignores layer 1 also ignores the weight meant to hold it down. Re-pointed from `_adopt_player`, lifted when the plank settles. |
| **X15** | **The paddy could not be crossed by anything the beat accepted.** Sub-beat 1 asked for Span, which after its own exclusions is a square or a triangle at 80 px, against 300 px of water. The only class long enough is `bridge`, and `bridge` is what the sub-beat excludes. `run_walk_level1` had been proving the crossing opened *with a bridge* the whole time. | Roll first, at the water, and Span second, at the stair. The plank is the crossing, and the suite walks the real loop. |
