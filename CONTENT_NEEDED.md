...
| **Level 2 plaza** | **AUTHORED 8-BIT** (2026-09-01) | `tools/build_plaza_art.py` → `game/assets/Level2/plaza/` | Themed on the **Basilica del Santo Niño, Cebu** — coral stone in three tiers, the belfry beside it, bahay na bato, an arcade, Sinulog red and gold. **Replaces the delivered `Level2_CompletedLook` as the playable backdrop**, which was Pahiyas (Lucban) and is a painterly vista rather than a side-scrolling set: it has a retaining wall behind the dancers and another in front, so a character standing between them read as two platforms. The painting stays in the repo and still backs the hub card. **The no-dancers-variant blocker is gone**: the dancers are sprites now, so the Protector route's scare is finally visible. |
