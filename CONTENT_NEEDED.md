...
| **Level 2's four insides and their contents** | **BUILT FROM THE TILESET** (2026-08-30) | code-drawn, `piyesta_room_2d.gd`, `church_interior_2d.gd`, `piyesta_door_2d.gd`, `kandila_2d.gd` | The delivered set is the plaza and nothing else. The design lists all of this under what does not exist: the **church interior** (nave, altar, candle rack, pews, window light — `MG_Church` is a facade), **Alley 1 and Alley 2** (background, midground, foreground, ground), a **dark-palette variant of `TextureMap_Piyesta`**, the **house door set** (closed / lit from inside / keyhole / open) and its small interior, and the **kandila** in three states. The rooms are no longer hand-drawn: `tools/build_tiles.py` cuts `TextureMap_Piyesta` into its three nine-slice wall sets, the plaza floor, stucco, roof, plank, thatch, stairs and props, and the rooms are built from those — the same pixels as the plaza. **What is still missing is the dark-palette variant of the tileset the design asks for**: the alleys are the daylight moss set under a blue tint, which is honest about the light but is not the same as art made for shade. The church interior set, the alley layer sets and the kandila's three states are still owed. |
| **`MG_People`, no-dancers variant** | **MISSING, and it blocks a visible consequence** | `game/assets/Level2/` | The four dancers are painted into `mg_people.png`. Problem 1's Protector route scares them off the plaza permanently, and the plate cannot give them up: a bounding-box cut takes the palm trunks standing behind two of them and smearing the hole closed leaves streaks, loose feet and a floating flower; a colour mask keyed on the white saya lifts the dress and leaves the hats, faces, arms, hands, fans and shoes. **Both were tried.** Until the variant exists, `DancerGroup2D` draws nothing and the scare is mechanically complete but visually invisible. Also needed with it: the four dancers as their own sprites, with a flee cycle and a hand-over-candle pose. |
| **LOLOGHOST — praying** | **MISSING, and Scene 2 is built on it** | `game/assets/.../LOLOGHOST` | The delivered sheet has float, hurry, still, wave, cheer and a five-frame turnaround. Nothing in it substitutes: `cheer` is arms-up celebration and reads as encouragement, which is worse than nothing at an altar. Until it lands, `level_2.gd` holds him still at the rack and lets the dialogue carry the beat — deliberately, rather than faking it with a pose that means something else. The design also names **LOLOGHOST — laughing**, which fires on every restriction violation and is the most-seen pose in the level. |
| **Levels 3–5 art** | **MISSING** | `game/assets/Level<N>/` | Dagat (storm sea, Coron cliffs) · Dilim (pitch-black forest, bioluminescence) · Mayon (erupting paint). Nothing exists for any of them. |
