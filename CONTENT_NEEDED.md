...
| **Level 2's four insides and their contents** | **PLACEHOLDER** (2026-08-30) | code-drawn, `piyesta_room_2d.gd`, `church_interior_2d.gd`, `piyesta_door_2d.gd`, `kandila_2d.gd` | The delivered set is the plaza and nothing else. The design lists all of this under what does not exist: the **church interior** (nave, altar, candle rack, pews, window light — `MG_Church` is a facade), **Alley 1 and Alley 2** (background, midground, foreground, ground), a **dark-palette variant of `TextureMap_Piyesta`**, the **house door set** (closed / lit from inside / keyhole / open) and its small interior, and the **kandila** in three states. All of it is drawn in code for now, at seventy-two pixels to the metre, to `ART_PLACEHOLDERS.md` rules. |
