...
| **Lolo's dialogue portrait** | **DONE** (2026-08-25) | `game/assets/characters/lolo/lolo_portrait.png` | His sheet carries a hero card like the apo's — 205 x 303, a different drawing rather than the world sprite enlarged — so `dialogue_portrait.gd` cuts a bust from it the same way. The SubViewport that used to rasterise the code-drawn blob at its own size and blow it back up is gone, and with it the half-height rule that kept it from filling the corner. The two speakers differ in one number now: where the bust is cut. The apo's lands at the hip, Lolo's where his tail starts, because his head is two fifths of him. |
